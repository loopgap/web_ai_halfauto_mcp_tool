...
- [TODO] 是否定义输入、输出、失败路径。
- [TODO] 是否定义错误码与恢复动作。
- [TODO] 是否接入审计事件与 trace。
- [TODO] 是否补齐单测/集成/E2E/回归。
- [TODO] 是否通过策略门禁与发布门禁。
- [TODO] 是否更新 route、runbook、benchmark。
- [TODO] 是否覆盖识别准确度专项（指纹/预检查/两阶段/归档）。

## 19. v0.3 最终统一验收

- [TODO] 前端核心页面全部状态机化，失败可恢复。
- [TODO] 规则引擎可解释且具备冲突治理。
- [TODO] 本地小模型分层可运行并可降级。
- [TODO] Skill v3 / Workflow v3 全量落地且可视化编辑。
- [TODO] 安全、审查、测试、发布、回滚形成单一闭环。
- [TODO] 每次新增工程均执行逻辑闭环并沉淀证据。
- [TODO] GPU 优先运行稳定可用，且 NPU 在可用时可无缝切换并满足质量/资源双约束。
- [TODO] 识别准确度与交互准确度专项全部达标（向导、预检查、两阶段、事务、归档）。
...
- [TODO] 全局 token 无硬编码污染。
- [TODO] 核心组件状态齐全（含 loading/error/disabled）。
- [TODO] 关键页面在 `sm/md/lg` 无错位、无截断异常。
- [TODO] Error Boundary 与空态兜底生效。
- [TODO] 错误文案使用统一字典并展示 trace_id。
- [TODO] 视觉回归测试通过。
...
- [TODO] `pnpm exec tsc --noEmit` 通过。
- [TODO] `cargo test --manifest-path src-tauri/Cargo.toml` 通过。
- [TODO] `pnpm ci:governance` 通过并生成证据包。
- [TODO] 错误码契约测试通过（后端错误码、catalog、前端恢复映射三方一致）。
- [TODO] 归档一致性检查通过（run/artifact/dispatch_trace 可互相追溯）。
- [TODO] 三个关键页面文案与状态快照通过（Console/Targets/Settings）。
...
- [x] `cargo test` → 全部通过。
- [x] 非预设浏览器打开时，ConsolePage 显示琥珀色告警。
- [x] 告警提供"继续使用"和"进入绑定向导"两个可操作按钮。
- [x] `detect_browsers` 命令可枚举已安装浏览器并标记未知类型。
...
- [x] `tsc --noEmit` → 0 errors（测试文件除外）
- [x] 切换页面时有淡入动画
- [x] 搜索栏可点击打开命令面板
- [x] 首次加载显示 splash screen 后过渡到应用
- [x] Dashboard 重渲染次数显著减少（React DevTools 验证）
